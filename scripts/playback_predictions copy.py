...
def run_playback(
    ...
    #     target_tcp_quat = tcp_quat[t]   # [w, x, y, z]

    #     for _ in range(substeps_per_frame):
    #         tcp_pose = base_env.agent.tcp.pose

    #         # pose.p : (1, 3)
    #         cur_xyz  = tcp_pose.p[0].cpu().numpy().astype(np.float32)
    ...
    #             current_xyz=cur_xyz,
    #             current_quat=cur_quat,
    #             target_xyz=target_tcp_xyz,
    #             target_quat=target_tcp_quat,
    #             gain_xyz=gain_xyz,
    #             gain_rot=gain_rot,
    #         )
    ...
    env.close()
    print(f"\nPlayback done.  Total fail frames: {fail_count}")
