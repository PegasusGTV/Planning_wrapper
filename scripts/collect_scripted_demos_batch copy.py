    ...
    def _thunk():
        ...
        class _InitWrapper(gym.Wrapper):
            ...
            def reset(self, **kwargs):
                ...
                # Default env reset (sets z-heights, physics, etc.)
                obs, info = self.env.reset(**kwargs)
                self._cache_z()

                # Apply custom init — this happens BEFORE RecordEpisode records
                # obs[0], since RecordEpisode now wraps this class (not the other
                # way around). The recorded frame 0 will reflect these positions.
                ...
                # Re-read obs from the simulator so it reflects the custom positions
...
def run_gpu(args: argparse.Namespace):
    ...
    def init_env_gpu(i: int):
        ...
        n_dof   = int(base_env.agent.robot.dof.item())
    ...
    while completed.sum() < args.num_demos:
        ...
        for i in np.where(dones)[0]:
            ...
            alive_steps_arr[i] = 0
            if n_done >= args.num_demos:
                break
            env.reset(env_idx=i)   # ManiSkill per-env reset
            init_env_gpu(i)
