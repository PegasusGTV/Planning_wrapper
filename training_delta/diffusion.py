...
class DDIM:
    """
    DDIM with M0 (sample / clean-signal) prediction, matching MAGNet.

      Forward:  x_t = √ᾱ_t · x_0  +  √(1−ᾱ_t) · noise
      Target:   x_0  (the clean signal directly)
    """
    ...
    def __init__(
        ...
        self.scheduler = DDIMScheduler(
            num_train_timesteps=num_train_timesteps,
            beta_schedule=beta_schedule,
            prediction_type="sample",   # M0 prediction
            clip_sample=clip_sample,
            clip_sample_range=clip_sample_range,
        )
        # Keep on CPU; moved to device lazily in _alphas_at
    ...
    def get_target(
        ...
        x_0:       torch.Tensor,
        noise:     torch.Tensor,
        timesteps: torch.Tensor = None,   # unused for M0
    ) -> torch.Tensor:
        return x_0   # M0 prediction always targets the clean signal