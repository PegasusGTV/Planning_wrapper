...
def evaluate_target(
    ...
    env      = _build_env(render, target_xy=target_xy, video_dir=video_dir, video_name=video_name)
    base_env = _unwrap(env)
    env.reset()
    _setup_episode(base_env, initial_block_pos, initial_block_quat, initial_gripper_xy)
...
def load_raw_npz(npz_path: str, conversion_mode: str) -> dict:
    ...
    conversion_mode : str   '3d' (or '2d' once implemented).
...
def print_target_results(results: dict, success_radius: float) -> None:
    print("\n── Target Evaluation ───────────────────────────────────────────")
    print(f"  Success (r={success_radius*100:.1f} cm)  : {'✓' if results['success'] else '✗'}")
...
def parse_args():
    ...
    p.add_argument("--success_radius", type=float, default=0.05,
                   help="Success radius in metres for --mode target.")
    p.add_argument("--playback_mode",  type=str,   default="action",
...
def main():
    ...
    if args.h5 is not None:
        ...
        print(f"Loading raw NPZ: {args.raw_npz}  conversion_mode={args.conversion_mode}")
        ...
        target_xy = (np.array(args.target_xy, dtype=np.float32)
    ...
    if args.mode == "target":
        ...
        if predicted_block_pos is None:
            print("Warning: no predicted block positions available — "
