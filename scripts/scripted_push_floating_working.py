...
STANDOFF_DIST_MIN = 0.05
STANDOFF_DIST_MAX = 0.12
STANDOFF_DIST_MU  = 0.08   # Gaussian centre
STANDOFF_DIST_STD = 0.02
...
def _jittered_corners(block_xy: np.ndarray, hx: float, hy: float,
    ...
    for sx in (+1, -1):
        for sy in (+1, -1):
            ...
            corners.append(_clamp_xy(np.array([wx, wy])))
    return corners
...
class FaceApproachPushPolicy:
    ...
    MAX_STEP             = 0.01
    ARRIVE_THRESH        = 0.015
    # Resample interval drawn from U[RESAMPLE_MIN, RESAMPLE_MAX] each time
    RESAMPLE_INTERVAL_MIN = 20
    RESAMPLE_INTERVAL_MAX = 40
    # Gripper is considered "in contact" when this close to block surface
    CONTACT_DIST         = 0.04   # metres from block centre to gripper centre
    ...
    def __init__(self):
        ...
        self._step             : int              = 0
        self._next_resample    : int              = 0   # step count at which to resample
    ...
    def reset(self, ee_xy: np.ndarray, block_xy: np.ndarray,
        ...
        self._rng          = rng
        self._step         = 0
        push_dir = self._new_face(ee_xy, block_xy, block_yaw, prev_face=None)
    ...
    def _in_contact(self, ee_xy: np.ndarray, block_xy: np.ndarray) -> bool:
        """True if the gripper is close enough to the block to be pushing."""
    ...
    def _standoff_point(self, block_xy: np.ndarray,
        ...
        """Standoff: behind the face by _standoff_dist, shifted laterally by face_offset."""
        return _clamp_xy(
            block_xy
            - push_dir    * self._standoff_dist
            + tangent_dir * self._face_offset
        )

    def _new_face(self, ee_xy: np.ndarray, block_xy: np.ndarray,
                  block_yaw: float,
                  prev_face: int | None) -> np.ndarray:
        """
        Sample a new face, style, standoff, and contact offset.

        If the same face index is drawn AND the gripper is already in contact,
        skip repositioning and stay in push phase.  Otherwise plan the approach.
        """
        self._style       = self._rng.choice(["face", "directed"])
        new_face_index    = int(self._rng.integers(4))
        ...
        # Jittered standoff distance and lateral offset (always resample these)
        ...
        self._face_offset = _truncated_gaussian(
            self._rng, mu=0.0, sigma=max_offset * 0.45,
            lo=-max_offset, hi=max_offset
        )

        if same_face and in_contact:
            # Already on this face and touching — no need to reposition
            self._phase     = "push"
            self._waypoints = []
            print(f"  [{self._style:8s}] face {self._face_index} (same, in contact) "
                  f"→ continue push  offset={self._face_offset:+.3f}m  "
            ...
            reason = "same face, no contact" if same_face else "new face"
    ...
    def act(self, ee_xy: np.ndarray, block_xy: np.ndarray,
        ...
        # Only count steps spent actually pushing — not repositioning time.
        # This prevents the resample timer from firing mid-approach and
        # trapping the gripper in an infinite repositioning loop.
        if self._phase == "push":
            self._step += 1
            if self._step >= self._next_resample:
