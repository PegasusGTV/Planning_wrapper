...
def run_inference(
    ...
    #         clip_val = 2.0  # match your config's clip_sample_range
    #         s_x0 = s_x0.clamp(-clip_val, clip_val)
    #         a_x0 = a_x0.clamp(-clip_val, clip_val)
    ...
    # device = seq.device
    # # Set up scheduler
    # diffusion.scheduler.set_timesteps(50, device=device)
    # timesteps = diffusion.scheduler.timesteps

    # # Start from pure noise
    # s_t = torch.randn((B, T, SD), device=device)
    # a_t = torch.randn((B, T, AD), device=device)

    # # Denoising loop
    # for t in timesteps:
    #     t_batch = t.expand(B)
    #     model_output = model(s_t, a_t, t_batch)
    ...
    # s_mse_norm = F.mse_loss(pred_raw[..., :SD], gt_raw[..., :SD]).item()
    # a_mse_norm = F.mse_loss(pred_raw[..., SD:], gt_raw[..., SD:]).item()
    # return {
    ...
    # Normalize GT for MSE comparison
    s_0, a_0 = normalize(seq.to(device), norm, SD)

    # Set up scheduler
    diffusion.scheduler.set_timesteps(n_steps, device=device)
    timesteps_sched = diffusion.scheduler.timesteps  # avoid shadowing outer variable

    # Start from pure noise — work with concatenated tensor so scheduler handles it cleanly
    x_t = torch.randn((B, T, D), device=device)  # [B, T, SD+AD]

    for t in timesteps_sched:
        t_batch = t.view(1, 1).expand(B, T)  # [B, T]

        s_t = x_t[..., :SD]
        a_t = x_t[..., SD:]

        s_pred, a_pred = model(s_t, a_t, t_batch)
        eps_pred = torch.cat([s_pred, a_pred], dim=-1)  # [B, T, D]
    ...
    # x_t is now the denoised prediction in normalized space
    s_final = x_t[..., :SD]
    a_final = x_t[..., SD:]

    s_mse_norm = F.mse_loss(s_final, s_0).item()
    a_mse_norm = F.mse_loss(a_final, a_0).item()

    pred_raw = denormalize(s_final, a_final, norm)
    gt_raw   = seq.to(device)
